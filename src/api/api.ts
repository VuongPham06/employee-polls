...
export function getUserAnswered(userId: string): Promise<Record<QuestionId, AnswerId>> {
  return new Promise<Record<QuestionId, AnswerId>>((resolve, reject) => {
    const user = users[userId];
    if (!user) {
      reject(`User not found.`);
    }
    setTimeout(() => resolve(user.answers), 100);
  });
}

export function saveUser(user: { id: string; password: string; name: string; avatarURL?: string }) {
  return new Promise<User>((resolve, reject) => {
    const { id, password, name, avatarURL } = user;
    if (!id || !name || !password) {
      reject('Please provide id, name, password');
    } else if (Object.prototype.hasOwnProperty.call(users, id)) {
...
export function saveQuestion(question: {
  ...
  return new Promise<Question>((resolve, reject) => {
    ...
    } else if (!Object.prototype.hasOwnProperty.call(users, author)) {
...
export function saveQuestionAnswer(answer: {
  ...
  return new Promise<boolean>((resolve, reject) => {
    ...
    } else if (!Object.prototype.hasOwnProperty.call(users, authedUser)) {
      reject(`User id '${authedUser}' does not exist`);
    } else if (!Object.prototype.hasOwnProperty.call(questions, qid)) {
